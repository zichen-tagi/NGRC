%% Representative multi-lattice optical NGRC fusion demo
% This script demonstrates multi-lattice feature fusion using synthetic
% placeholder data. It is intended to show the analysis workflow and does not
% reproduce the experimental numerical results reported in the manuscript.
...
%% Generate single-lattice and fused multi-lattice feature matrices
% In the experiment, each lattice condition is generated by changing the phase
% or amplitude modulation in the loop. Here, lattice_id changes the deterministic
% mixing map used to synthesize optical-like feature matrices. The per-lattice
% feature count is kept modest so that one synthetic lattice does not saturate
% the task before the complementary lattice projections are added.
num_features_per_lattice = 15;
num_lattices = 3;

single_feature_matrix = make_lattice_features(delayed_inputs, num_features_per_lattice, 1);
feature_blocks = cell(1, num_lattices);
for lattice_id = 1:num_lattices
    feature_blocks{lattice_id} = make_lattice_features( ...
        delayed_inputs, num_features_per_lattice, lattice_id);
end
fused_feature_matrix = [feature_blocks{:}];

%% Train ridge readouts and compare NMSE
...
fprintf('\nMulti-lattice optical NGRC fusion demo\n');
fprintf('Single-lattice NMSE: %.4f\n', single_nmse);
fprintf('Multi-lattice fused NMSE: %.4f\n', fused_nmse);
fprintf('Relative NMSE change: %.2f%%\n', ...
    100 * (fused_nmse - single_nmse) / single_nmse);
fprintf('\nSingle lattice:\n');
fprintf('D = %d\n', single_stats.D);
...
title('Single-lattice and multi-lattice prediction traces');
grid on;
